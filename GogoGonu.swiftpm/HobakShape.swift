    ...
    var body: some View {
        GeometryReader { geometry in
            Path() { path in
                ...
                path.move(to: CGPoint(x: width / 2 - radius - lineSpace, y: height / 2 - radius - lineSpace))
                path.addLine(to: CGPoint(x: width / 2 + radius + lineSpace, y: height / 2 - radius - lineSpace))
